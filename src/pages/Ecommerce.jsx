import React from 'react';
import { GoPrimitiveDot } from 'react-icons/go';
import { Button } from '../components';
import SparkLineChart from '../components/Charts/SparkLineChart';
import StackedChart from '../components/Charts/StackedChart';
import { useStateContext } from '../contexts/ContextProvider';
import { earningData, SparklineAreaData, ecomPieChartData } from '../data/dummy';


const Ecommerce = () => {
  return (
    <div className='mt-12'>
      <div className="flex flex-wrap lg:flex-nowrap justify-center">
        <div className="bg-white dark:text-gray-200
        dark:bg-secondary-dark-bg h-44
        rounded-xl w-full lg:w-80 p-8 pt-9 m-3
        bg-hero-pattern bg-no-repat bg-cover bg-center">
          <div className="flex justify-between items-center">
            <div className="">
              <p className='font-bold text-gray-400'>Ganancias</p>
              <p className='text-2xl'>$99999</p>
            </div>
          </div>
          <div className="mt-6">
            <Button
              color="white"
              bgColor="blue"
              text="Descargar"
              borderRadius="10px"
              size="md"
            />
          </div>
        </div>
        <div className="flex m-3 flex-wrap justify-center gap-1 items-center">
          {earningData.map((m) => (
            <div
              key={m.title}
              className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg md:w-56
              p-4 pt-9 rounded-2xl"
            >
              <button type="button" 
                style={{ color: m.iconColor, backgroundColor:m.iconBg}}
                className="text-2xl opacity-0.9 rounded-full p-4 hover:drop-shadow-xl">
                {m.icon}
              </button>
              <p className='mt-3'>
                <span className='text-lg font-semibold'>
                  {m.amount}
                </span>
                <span className={`text-sm text-${m.pcColor} ml-2`}>
                  {m.percentage}
                </span>
              </p>
              <p className='text-sm text-gray-400 mt-1'>{m.title}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="flex gap-8 flex-wrap justify-center">
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg m-3 rounded-2xl md:w-780">
          <div className='flex justify-between mx-4 my-2'> 
            <p className='font-semibold text-xl'>Novedades ganancias</p>
            <div className="flex items-center gap-4">
              <p className='flex items-center gap-2 text-gray-600 hover:drop-shadow-xl'>
                <span>
                  <GoPrimitiveDot/>
                </span>
                <span>
                  Gastos
                </span>
              </p>
              <p className='flex items-center gap-2 text-green-400 hover:drop-shadow-xl'>
                <span>
                  <GoPrimitiveDot/>
                </span>
                <span>
                  Ganancias
                </span>
              </p>  
            </div>
          </div>
          <div className="mt-10 flex gap-10 flex-wrap justify-center">
            <div className="border-r-1 border-color m-4 pr-10">
              <div>
                <p>
                  <span className='text-3xl font-semibold'>56,000</span>
                  <span className='p-1.5 hover:drop-shadow-xl cursor-pointer rounded-full
                  text-white bg-green-400 ml-3 text-xs'>10%</span>
                </p>
                <p className='text-gray-500 mt-1'>Presupuesto</p>
              </div>
              <div className='mt-8'>
                <p>
                  <span className='text-3xl font-semibold'>28,500</span>
                </p>
                <p className='text-gray-500 mt-1'>Gasto</p>
              </div>
              <div className="mt-5">
                <SparkLineChart
                  currentColor="blue"
                  id="line-sparkline"
                  type="Line"
                  height="80px"
                  width="250px"
                  data={SparklineAreaData}
                  color="blue"
                />
              </div>
              <div className='mt-10'>
                <Button
                  color="white"
                  bgColor="blue"
                  text="Descargar reporte"
                  borderRadius="10px"
                />
              </div>
            </div>
            <div className="">
              <StackedChart 
                width="320px"
                height="360px"
              /> 
            </div>
          </div>  
        </div>
      </div>
    </div>
  )
}

export default Ecommerce